...
class Simulasi extends Component
{
    ...
    {
        if ($this->session->jenis_sesi !== 'Simulasi') {
            redirect()->route('user.dashboard')->with('error', 'Sesi ini bukan ujian simulasi.');
        }
        ...
        if ($now->lt($mulai)) {
            redirect()->route('user.dashboard')->with('error', 'Sesi simulasi ini belum dimulai. Silakan kembali pada jam ' . $mulai->format('H:i') . '.');
        }

        if ($now->gt($selesai)) {
            redirect()->route('user.dashboard')->with('error', 'Mohon maaf, waktu pengerjaan untuk sesi simulasi ini telah berakhir (Terlambat).');
        }

        // Simulasi might allow multiple attempts, or maybe not. Let's just keep the single attempt rule matching Examination for now.
        ...
        if ($finishedAttempt) {
            redirect()->route('user.dashboard')->with('error', 'Anda sudah melakukan simulasi pada sesi ini.');
        }
    }
    ...
    public function render(): View
    {
        ...
        return view('livewire.student.simulasi', [
            'answeredCount' => $answeredCount,
            'progress' => $progress
        ])->layout('components.layouts.app', ['title' => 'Simulasi: ' . $this->session->test->nama_tes]);
    }
}
