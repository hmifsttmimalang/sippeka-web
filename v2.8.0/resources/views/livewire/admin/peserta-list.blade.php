                    ...
                    <div class="card shadow mb-4">
                        <div class="card-header py-3 flex justify-between items-center">
                            ...
                            <div class="flex items-center space-x-2">
                                <input type="text" wire:model.live="search" class="form-control form-control-sm" placeholder="Cari nama...">
                            </div>
                        </div>
                        <div class="card-body">
                                ...
                                <table class="table table-bordered table-hover">
                                    ...
                                    <tbody>
                                        @foreach ($listPendaftar as $item)
                                            <tr class="text-center">
                                                <td>{{ ($listPendaftar->currentPage() - 1) * $listPendaftar->perPage() + $loop->iteration }}</td>
                                                <td class="text-left font-weight-bold">{{ $item->nama }}</td>
                                                <td>{{ $item->skill->nama ?? '-' }}</td>
                                                <td><span class="badge badge-info">{{ $item->nilai_keahlian ?? '0' }}</span></td>
                                                <td><span class="badge badge-primary">{{ $item->nilai_wawancara ?? '0' }}</span></td>
                                                <td>
                                                    <span class="badge {{ $item->rata_rata >= 70 ? 'badge-success' : 'badge-warning' }}">
