<div @class([
    ...
    'bg-[#F8F9FF]' => $session->session_type === 'Selection',
    'bg-[#899bbd]' => $session->session_type === 'Simulation'
])>
    <style>
        ...
        <header class="sticky top-0 z-50 h-20 premium-glass px-6 lg:px-12 flex items-center justify-between animate-header">
            <div class="flex items-center space-x-5">
                ...
                <div class="hidden sm:block">
                    <h2 class="text-lg font-black text-slate-900 tracking-tight font-outfit uppercase leading-tight">{{ $session->test->name }}</h2>
                    <div class="flex items-center space-x-3 mt-1">
                        ...
                        <div class="flex items-center space-x-1.5">
                            ...
                            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Bidang {{ $session->test->category->name ?? 'Umum' }}</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-12 relative z-10">
                    ...
                    <div class="space-y-4">
                        <h2 class="text-4xl lg:text-5xl font-black text-slate-900 font-outfit uppercase tracking-tighter leading-none">Examination Report</h2>
                            <span>Sesi {{ $session->session_type === 'Selection' ? 'Seleksi' : 'Simulasi' }}</span>
                            <span class="w-1.5 h-1.5 bg-slate-200 rounded-full"></span>
                            <span>{{ $session->test->name }}</span>
                        </div>
                    </div>
                             ...
                             <div class="grid grid-cols-1 gap-8">
                                @foreach($questions as $index => $q)
                                    @php
                                        ...
                                        $isCorrect = $userChar === $q->correct_answer;
                                    @endphp
                                    <div @class([
                                        ...
                                        <div class="flex items-start justify-between space-x-6 mb-8 px-2">
                                            <p class="text-xl font-bold text-slate-800 leading-[1.5] font-outfit">{{ $index + 1 }}. {{ $q->question_text }}</p>
                                            <div @class([
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-10 pt-8 border-t border-black/5">
                                            <div class="px-4">
                                                ...
                                                <p @class(['font-black text-base font-outfit', 'text-emerald-700' => $isCorrect, 'text-rose-700' => !$isCorrect && $userChar !== null, 'text-slate-400 italic' => $userChar === null])>
                                                    {{ $userChar ? strtoupper($userChar) . '. ' . ($q->{"option_".strtolower($userChar)} ?? '') : 'None' }}
                                                </p>
                                            </div>
                                            <div class="px-4">
                                                <span class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] block mb-2 italic">Correct Key</span>
                                                <p class="font-black text-base font-outfit text-emerald-700">{{ strtoupper($q->correct_answer) . '. ' . ($q->{"option_".strtolower($q->correct_answer)} ?? '') }}</p>
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
